...
# heavily based on http://www.subsubroutine.com/sub-subroutine/2016/11/12/painting-like-van-gogh-with-convolutional-neural-networks
...
logger = get_logger('deepart')

def create_painting(file_content_image : str, 
                    file_style_image : str, 
                    n_checkpoints : int =20,
                    max_pix_length : int =1000, # size of the largest axis of the image
                    path_results: str ='results', 
                    path_output : str ='output',
                    path_VGG19 : str = 'imagenet-vgg-verydeep-19.mat'):
    """Wrapper function around creating a painting
    Make sure you've download VGG from https://www.kaggle.com/teksab/imagenetvggverydeep19mat.
    
    :param str content_pic_name: path of the picture you want in a certain style
    :param str style_pic_name: name of the painting containing the style
    :param int n_checkpoints: number of checkpoints
    :param int max_pix_length: size of the largest axis of the image
    :param str path_results: path for the results
    :param str path_output: path for the temporary outputs
    :param str path_VGG19: path to VGG
    
    Example:
    create_painting('portrait.jpg','picasso.jpg')
    """
    logger.info(f'\n----starting to paint {file_content_image} in the style of {file_style_image}')
    ...
    assert os.path.isfile(path_VGG19), 'Image net not found, download from https://www.kaggle.com/teksab/imagenetvggverydeep19mat'
    ...
    if shape[0] > max_pix_length or shape[1] > max_pix_length:

        if shape[0] < shape[1]:
            mp = shape[1] *1.0 / max_pix_length
        else:
            mp = shape[0] *1.0 / max_pix_length


        new_im_size = (int(shape[0] / mp), int(shape[1] / mp))
        print('resize to {}'.format(new_im_size))
        img_content = scipy.misc.imresize(img_content, new_im_size)
    ...
    with tf.Session() as sess:
        ...
        img_output = imgunprocess(img_output)
        timestr = time.strftime("%Y%m%d_%H%M%S")
        result_file = '{}/{}_{}_{}_{}'.format(path_results, timestr, i * n_iterations_checkpoint, content_pic_name.replace('.jpg','').replace('.jpeg',''), style_pic_name)
        imsave(result_file, img_output)

    logger.info(f'{result_file} is done')

if __name__ == '__main__':
    
    create_painting(
        file_content_image = sys.argv[1],
        file_style_image = sys.argv[2]
            )